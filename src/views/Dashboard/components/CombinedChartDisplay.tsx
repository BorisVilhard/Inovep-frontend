...
const CombinedChartDisplay: React.FC<CombinedChartDisplayProps> = ({
  ...
  const categoryData = localDashboardData.find((cat) => cat.categoryName === documentCategory);
              ...
              .map((item) => (
                <div key={item.id} className="m-1 flex items-center">
                  <input
                    ...
                    onChange={() => handleCheck(documentCategory, item.id, combinedChart.id)}
                  />
                  <span
                    className="ml-2 text-sm font-bold"
                    style={{
                      color: getTitleColors(combinedChart.data)[item.data[0].title],
                    }}
                  >
                    {item.data[0].title}
                  </span>
